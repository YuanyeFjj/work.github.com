...
function Slider(id, autosliding) {
	...
	this.start = function() {
		this.view.style.width = this.width  / the.cols * this.pages.length + 'px';
		this.view.style.height = this.height + 'px';
		
		for (var i=0; i<this.pages.length; i++) {
			this.pages[i].style.width = this.width / the.cols + 'px';
			this.pages[i].style.height = this.height + 'px';
		}		
		
		this.indicator.style.marginLeft = -19 * this.buttons.length / 2	+ 'px';
		for (var i=0; i<this.buttons.length-2; i++) {
	...
	this.next = function() {
		if (the.leftward) {
			...
			if (++the.index == Math.floor(the.pages.length/3) * 3-1)
				the.leftward = true;
		}
		
		the.indicate(the.index);
		the.view.style.left = -the.index * the.width  / the.cols + 'px';
	...
	this.select = function() {
		...
		if (the.index == 0) {
			the.leftward = false;
		} else if(the.index == Math.floor(the.pages.length/3) * 3-1) {
			the.leftward = true;
		}
